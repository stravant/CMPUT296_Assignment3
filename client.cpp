...
void loop() {
    ...
    // If we are making a request to find a shortest path, we will send
    // out the request on the serial port and then wait for a response 
    // from the server.  While this is happening, the client user interface
    // is suspended.

    // Simple example of seeing if there is some input waiting at the serial 
    // port that we should process.
    if ( Serial.available() ) {
        ...
        // the field extracted
        const uint8_t field_size = 10;
        char field[field_size];

        // read the line
        uint16_t bytes_read = serial_readline(line, line_size);
        ...
        // now extract out each of the " " delimited numbers on the line
        uint16_t field_index = 0;   // start at first field
        while (field_index < bytes_read ) {
        // for (uint8_t i=0; i < 5; i++) {
            ...
            int32_t field_value = string_get_int(field);
            Serial.print(" value: ");
...
uint32_t button_prev_push_time = 0;
...
uint8_t process_joystick(int16_t *dx, int16_t *dy) {
    ...
    *dy = joy_y / 128;
    ...
    if ( ! button_state ) {
        // if not pushed, return that immediately
        return 0;
        }

    // return the state of the button if pushed again after a suitable delay
    uint32_t cur_push_time = millis();
    if ( cur_push_time < button_prev_push_time ) {
        // time inversion caused by wraparound, so reset
        button_prev_push_time = cur_push_time;
    }
    if ( button_prev_push_time == 0 || 
             button_prev_push_time + button_sample_delay < cur_push_time ) {
...
const uint32_t bounce_delay = 2000;
